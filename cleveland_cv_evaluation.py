...
# ── Constants ──────────────────────────────────────────────────────────────────
N_FOLDS = 10
N_NUM = 13          # Number of numeric features (indices 0–13, scaled per fold)
...
def cv_hybrid_transformer(
    ...
    _banner("Hybrid Quantum-Classical Transformer (adaptive HEA)")
    ...
    for fold_i, (tr_idx, va_idx) in enumerate(skf.split(X_full, y_full), 1):
        ...
        # Data-adaptive circuit: size VQC capacity to training-set size
...
def _save_results(
    ...
    csv_path = RESULTS_DIR / "cleveland_cv_results.csv"
    df.to_csv(csv_path, index=False)
    print(f"\n  results/cleveland_cv_results.csv saved ({len(df)} models)")

    # mcnemar_result.txt (preserve existing format)
    p_val, stat, sig_str = mcnemar_info
    txt_path = RESULTS_DIR / "cleveland_mcnemar_result.txt"
    lines = [f"p_value={p_val}", f"statistic={stat}", f"result={sig_str}"]
    txt_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"  results/cleveland_mcnemar_result.txt saved")
    ...
    if mcnemar_abcd is not None:
        ...
        json_path = RESULTS_DIR / "cleveland_mcnemar_detail.json"
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(detail, f, indent=2)
        print(f"  results/cleveland_mcnemar_detail.json saved")
    ...
    if full_rows:
        ...
        full_csv = RESULTS_DIR / "full_metrics_cleveland.csv"
        df_full.to_csv(full_csv, index=False)
        print(f"  results/full_metrics_cleveland.csv saved ({len(df_full)} rows)")

    # ckd_fold_probas.npz — probability arrays for ROC/PR plotting
    if fold_probas:
        npz_path = RESULTS_DIR / "cleveland_fold_probas.npz"
        np.savez(npz_path, **{k: v for k, v in fold_probas.items() if v is not None})
        print(f"  results/cleveland_fold_probas.npz saved (keys: {list(fold_probas.keys())})")
...
def main() -> None:
    ...
    print("=" * 60)
    print("  10-FOLD STRATIFIED CROSS-VALIDATION — CLEVELAND HEART PIPELINE")
    ...
    for fname in ["cleveland_X_full.npy", "cleveland_y_full.npy"]:
        if not (DATA_DIR / fname).exists():
            print(f"ERROR: {DATA_DIR / fname} not found.")
            print("Run cleveland_preprocessing.py first to generate X_full.npy and y_full.npy.")
            sys.exit(1)

    X_full = np.load(DATA_DIR / "cleveland_X_full.npy")
    y_full = np.load(DATA_DIR / "cleveland_y_full.npy")
    ...
    if _HAS_STATS:
        ...
        try:
            ...
            stat_out = RESULTS_DIR / "cleveland_statistical_tests.json"
            save_statistical_tests(stat_results, str(stat_out))
            print(f"  results/cleveland_statistical_tests.json saved")
